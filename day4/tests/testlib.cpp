#define DOCTEST_CONFIG_IMPLEMENT_WITH_MAIN

#include <doctest/doctest.h>

#include "lib.hpp"

TEST_CASE("Sample input file length should be 11")
{
    const auto puzzle_input{ read_input("day4_sample_input.txt") };
    CHECK(puzzle_input.size() == 13);
}

TEST_CASE("Input file length should be 323")
{
    const auto puzzle_input{ read_input("day4_input.txt") };
    CHECK(puzzle_input.size() == 1159);
}

TEST_CASE("Part 1 answer with sample input should be 2")
{
    const auto puzzle_input{ read_input("day4_sample_input.txt") };
    const auto part1_answer{ process_passports(puzzle_input, problem_part::one) };
    CHECK(part1_answer == 2);
}

TEST_CASE("Part 1 answer should be 254")
{
    const auto puzzle_input{ read_input("day4_input.txt") };
    const auto part1_answer{ process_passports(puzzle_input, problem_part::one) };
    CHECK(part1_answer == 254);
}

TEST_CASE("Part 2 sample invalid passports should be invalid")
{
    const std::vector<std::string> puzzle_input{
        "eyr:1972 cid:100",
        "hcl:#18171d ecl:amb hgt:170 pid:186cm iyr:2018 byr:1926",
        "",
        "iyr:2019",
        "hcl:#602927 eyr:1967 hgt:170cm",
        "ecl:grn pid:012533040 byr:1946",
        "",
        "hcl:dab227 iyr:2012",
        "ecl:brn hgt:182cm pid:021572410 eyr:2020 byr:1992 cid:277",
        "",
        "hgt:59cm ecl:zzz",
        "eyr:2038 hcl:74454a iyr:2023",
        "pid:3556412378 byr:2007"
    };
    const auto part2_answer{ process_passports(puzzle_input, problem_part::two) };
    CHECK(part2_answer == 0);
}

TEST_CASE("Part 2 sample valid passports should be valid")
{
    const std::vector<std::string> puzzle_input{
        "pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980",
        "hcl:#623a2f",
        "",
        "eyr:2029 ecl:blu cid:129 byr:1989",
        "iyr:2014 pid:896056539 hcl:#a97842 hgt:165cm",
        "",
        "hcl:#888785",
        "hgt:164cm byr:2001 iyr:2015 cid:88",
        "pid:545766238 ecl:hzl",
        "eyr:2022",
        "",
        "iyr:2010 hgt:158cm hcl:#b6652a ecl:blu byr:1944 eyr:2021 pid:093154719"
    };
    const auto part2_answer{ process_passports(puzzle_input, problem_part::two) };
    CHECK(part2_answer == 4);
}

TEST_CASE("Part 2 answer should be 184")
{
    const auto puzzle_input{ read_input("day4_input.txt") };
    const auto part2_answer{ process_passports(puzzle_input, problem_part::two) };
    CHECK(part2_answer == 184);
}
